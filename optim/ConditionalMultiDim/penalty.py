"""Penalty functions method for constrained optimization."""

from typing import List, Callable

import numpy as np

from optim.UnconditionalMultiDim.gradient import conjugate_descent
...
def find_min(
    ...
    """Finds the minimum value of a function within a given interval with conditional constrains using the Penalty functions method.
    ...
    if equ_constrains is None:
        equ_constrains = []
    if comp_constrains is None:
        comp_constrains = []
    if equ_coeff is None:
        equ_coeff = np.ones(len(equ_constrains))
    ...
    def penalty_func(
        point: np.ndarray,
        alphas: np.ndarray,
        equ_coeff: np.ndarray,
        comp_coeff: np.ndarray,
    ) -> float:
        equ_penalty = np.sum(
            equ_coeff * np.array([constr(point) ** 2 for constr in equ_constrains])
        )
        comp_penalty = np.sum(
            comp_coeff
            * np.maximum(0, np.array([constr(point) for constr in comp_constrains])) ** 2
        )
        return func(point) + alphas[0] * equ_penalty + alphas[1] * comp_penalty

    def check_constrains(point: np.ndarray) -> bool:
        equ_values = np.array([constraint(point) for constraint in equ_constrains])
        comp_values = np.array([constraint(point) for constraint in comp_constrains])
        return np.all(np.abs(equ_values) < eps) and np.all(comp_values >= -eps)
