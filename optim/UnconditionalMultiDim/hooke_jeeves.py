...
def find_min(func: Callable[[np.ndarray], float], init_point: np.ndarray, alpha: float = 1.0,
             ...
             eps_func: float = 1e-5, max_iter: int = 100, trial_vector_decrease: float = 0.5,
    ...
    while (np.linalg.norm(previous_point - point) > eps and
        ...
        for dim in range(ndim):
            one_dim_func = lambda x, d=dim: func(
                np.array([val if i != d else x for i, val in enumerate(point + real_trial_vector)]))

            print(f"Trial vector: {trial_vector}")
            print(
                f"F(x): {one_dim_func(point[dim])}, F(x + h): {one_dim_func(point[dim] + trial_vector[dim])}, F(x - h): {one_dim_func(point[dim] - trial_vector[dim])}")

            if one_dim_func(point[dim] + trial_vector[dim]) > one_dim_func(point[dim]):
                if one_dim_func(point[dim] - trial_vector[dim]) > one_dim_func(point[dim]):
        ...
        if np.linalg.norm(previous_point - point) < 1e-6 and np.linalg.norm(trial_vector) > eps:
            print("Hello there")
