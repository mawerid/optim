...
from optim.utils.deriv import hessian, gradient
...
def find_min(func: Callable[[np.ndarray], float], init_point: np.ndarray,
  ...
  """Finds the minimum value of a function within a given interval using Newton method.
  ...
  while (np.linalg.norm(previous_point - point) > eps and
    ...
    grad = gradient(func, point, ndim, eps if eps < 1e-5 else 1e-5)
    hess = hessian(func, point, ndim, eps if eps < 1e-5 else 1e-5)

    if verbose:
      print(
        f"Gradient in current point: {np.linalg.norm(grad)}\nHessian matrix in current point: {hess}")

    delta = np.linalg.solve(hess, (-1) * grad)
    point += alpha * delta
