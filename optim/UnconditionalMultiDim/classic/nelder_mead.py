"""Nelder-Mead method for unconstrained optimization.
"""
...
def find_min(func: Callable[[np.ndarray], float], init_point: np.ndarray,
  ...
  """Finds the minimum value of a function using the Nelder-Mead method.
  ...
  while (np.linalg.norm(reflected - simplex[best_idx]) > eps and
    ...
    func_reflected = func(simplex[worst_idx])
    if func_reflected >= func_simplex[worst_idx]:
      simplex = simp.reduce(simplex, best_idx, split_length)
      length *= split_length
      func_reduced = func(simplex[worst_idx])

      if func_reduced >= func_simplex[worst_idx]:
        simplex = simp.squeeze(simplex, split_length)
        length *= split_length
...
def find_max(func: Callable[[np.ndarray], float], init_point: np.ndarray,
             ndim: int = 1, length: float = 10.0, split_length: float = 0.7, eps: float = 1e-3, max_iter: int = 1e5,
             verbose: bool = False, return_argmin: bool = True) -> np.ndarray:
  """Finds the maximum value of a function using the Nelder-Mead method.
